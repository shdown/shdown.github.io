...
class Context {
  ...
  async sleepMillis(ms) {
    ...
    for (; ms > maxLag; ms -= maxLag) {
      ...
      await (0, _utils.sleepMillis)(maxLag);
    }
...
const makeCheckbox = params => {
  const cb = (0, _utils.fromHtml)('<input type="checkbox"></input>');
  cb.setAttribute('id', params.id);
  cb.append(params.text);

  if (params.checked !== undefined) {
    cb.checked = params.checked;
  }
...
class FormView extends _view.View {
  constructor() {
    ...
    this._skipStatsCheckbox = makeCheckbox({
      id: 'fv-input-id-ss',
      text: (0, _gettext.__)('Skip gathering staticstics'),
      checked: false
    });
    ...
    this._form.appendChild(createFieldSet([this._skipStatsCheckbox]));
...
const asyncMain = async () => {
  ...
  const resolveDomainToId = async domain => {
    if (domain.match(/^-?[0-9]+$/) !== null) return parseInt(domain);
    let m = domain.match(/^.*\/(.*)$/);
  ...
  const work = async workConfig => {
    ...
    for (let i = 0; i < oids.length; ++i) {
      const oid = oids[i];
      const stat = stats[oid]; //if (stat === undefined)
      //    continue;
      ...
      if (stat !== undefined) implicitDenominator -= _progress_estimator.ProgressEstimator.statsToExpectedCommentsCount(stat, timeLimit);
...
class Transport {
  ...
  async callAPI(method, params) {
    ...
    try {
      ...
      throw new _vk_api.VkApiError(error_data.error_code, error_data.error_msg);
    }
